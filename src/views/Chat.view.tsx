...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    on('receive_message', onMessage);
    on('user_joined', onJoined);
    on('user_left', onLeft);

    return () => {
      off('receive_message', onMessage);
      off('user_joined', onJoined);
      off('user_left', onLeft);
    };
  }, [on, off, pushMessage]);
  ...
  const sendMessage = (e: FormEvent) => {
    ...
    if (!trimmed || !isConnected || trimmed.length > 500) return;
